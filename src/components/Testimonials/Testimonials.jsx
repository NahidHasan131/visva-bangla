  ...
  <div className="flex items-center gap-1">
    {[1, 2, 3, 4, 5].map((s) => (
      <button key={s} type="button" onClick={() => onChange(s)}
        ...
        <FaStar size={22} className={s <= value ? 'text-yellow-400' : 'text-gray-200'} />
      </button>
    ))}
  </div>
);
...
const ReviewCard = ({ review }) => (
  <div className="flex flex-col gap-4 bg-white rounded-2xl p-6 border border-gray-100 shadow-[0_2px_16px_rgba(0,0,0,0.06)] h-full">
    ...
    <p className="text-gray-600 text-sm leading-relaxed flex-1 italic">"{review.description}"</p>
    <div className="flex items-center gap-3 pt-3 border-t border-gray-100">
      <div className="w-10 h-10 rounded-full bg-secondary/15 flex items-center justify-center text-secondary font-bold text-sm shrink-0">
        {(review.title || review.name || '?').charAt(0).toUpperCase()}
      </div>
      <div>
        <p className="text-sm font-bold text-[#11141B] leading-none">{review.title || review.name}</p>
  ...
  return (
    ...
    >
      <div className="max-w-340 mx-auto px-6 lg:px-12">
        ...
        {/* Submit form */}
        <motion.div className="bg-white rounded-3xl border border-gray-100 shadow-[0_2px_24px_rgba(0,0,0,0.06)] overflow-hidden"
          variants={fadeUp} initial="hidden" whileInView="show" viewport={viewport}>
          <div className="flex flex-col lg:flex-row">

            {/* Left panel */}
            <motion.div className="lg:w-2/5 bg-secondary p-8 lg:p-10 flex flex-col gap-6 justify-center"
            ...
            {/* Right form */}
            <motion.div className="lg:w-3/5 p-8 lg:p-10"
