    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-xl self-start">
        <button type="button" onClick={() => setMode('url')}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${mode === 'url' ? 'bg-white text-[var(--color-secondary)] shadow-sm' : 'text-gray-500'}`}>
          <MdLink size={14} /> URL
        </button>
        <button type="button" onClick={() => setMode('file')}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${mode === 'file' ? 'bg-white text-[var(--color-secondary)] shadow-sm' : 'text-gray-500'}`}>
      ...
      {mode === 'file' && (
        <label className="cursor-pointer">
          <div className={`w-full h-36 rounded-xl border-2 border-dashed flex flex-col items-center justify-center gap-2 transition-colors ${
            value ? 'border-[var(--color-secondary)]' : 'border-gray-200 hover:border-[var(--color-secondary)]'
          }`}>
            {value ? (
