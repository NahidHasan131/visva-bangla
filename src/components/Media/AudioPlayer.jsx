...
const AudioPlayer = ({ tracks: rawTracks }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className={`w-12 h-12 rounded-xl overflow-hidden shrink-0 flex items-center justify-center ${isActive ? 'ring-2 ring-white/30' : ''}`}>
                {track.thumbnail ? (
                  ...
                  <div className={`w-full h-full flex items-center justify-center ${isActive ? 'bg-white/20' : 'bg-white'}`}>
                    {isActive && isPlaying
                      ? <FaPause size={14} className={isActive ? 'text-white' : 'text-secondary'} />
                      : <FaPlay size={14} className={isActive ? 'text-white' : 'text-secondary'} />}
                  </div>
                )}
              </div>
      ...
      {/* Player — keep dark bg style */}
      <div className="lg:w-80 shrink-0">
        <div className="bg-[#11141B] rounded-3xl p-8 flex flex-col gap-6 sticky top-24">

          {/* Album art */}
          <div className="flex flex-col items-center gap-3 text-center">
            <div className="w-24 h-24 rounded-full overflow-hidden bg-secondary/20 flex items-center justify-center">
              {current.thumbnail
                ? <img src={current.thumbnail} alt={current.title} className="w-full h-full object-cover" />
                : <MdMusicNote size={36} className="text-secondary" />}
            </div>
          ...
          <div className="flex items-center justify-center gap-6">
            ...
            <button onClick={togglePlay} className="w-14 h-14 rounded-full bg-secondary flex items-center justify-center hover:bg-secondary/80 transition-colors">
